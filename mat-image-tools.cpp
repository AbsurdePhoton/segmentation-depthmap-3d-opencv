...
Mat QImage2Mat(const QImage &source) // Convert QImage to Mat, every number of channels supported
{
    ...
    return {}; // return empty Mat if type not found
}
...
QPixmap Mat2QPixmapResized(const Mat &source, const int &width, const int &height, const bool &smooth) // convert Mat to resized QPixmap
{
    ...
    if (smooth) {
        quality = Qt::SmoothTransformation;
    }
    else {
        quality = Qt::FastTransformation;
    }
...
float EuclideanDistance(const Point& center, const Point& point, int radius) { // return distance between 2 points
    float distance = sqrt(std::pow(center.x - point.x, 2) + std::pow(center.y - point.y, 2));

    if (distance > radius) {
        return radius; // no value beyond radius
    } return distance;
}
        ...
        for (int row = area.y; row < area.y + area.height; row++) { // scan mask
            for (int col = area.x; col < area.x + area.width; col++) {
                if (msk.at<uchar>(row, col) != 0) { // non-zero pixel in mask
                    ...
                    if (C <= C1) {
                        CO = beginColor; // before begin point : begin color
                    }
                    else if (C >= C2) {
                        CO = endColor; // after end point : end color
                    }
                    else {
                        CO = round(GrayCurve(float(beginColor * (C2 - C) + endColor * (C - C1)) / (C2 - C1),
